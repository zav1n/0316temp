...
(function (window, undefined) {
  ...
  jQuery.fn = jQuery.prototype = { //工厂函数的原型
    ...
    init: function (seletor) { // seletor (用户写的选择器 => jQuery工厂 => jQuery.fn.init中) 你传入的参数
      ...
      switch (Object.prototype.toString.call(seletor)) {
        case "[object String]":
          // 有没有筛选器
          if (seletor.indexOf(":") !== -1) {
            let filterReg = /\:(.+)[(](.+)[)]/
            //函数名称
            let fnName = seletor.match(filterReg)[1];
            //函数的参数
            let fnAttr = seletor.match(filterReg)[2];
            seletor = seletor.slice(0, seletor.indexOf(":")) //"div"
            this.arr = Array.from(document.querySelectorAll(seletor));
            //这次查询到的数组的长度
            this.length = this.arr.length;
            //将原本长度为3的数组 修改成长度为1的数组
            this.arr = this[fnName](fnAttr)
            //准备去正则匹配去取他使用的筛选器是什么
          } else {
            this.arr = Array.from(document.querySelectorAll(seletor));
            //这次查询到的数组的长度
    ...
    eq: function (index) {
      //将他的长度变为1
      this.length = this.arr[index] ? 1 : 0;
